...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Account created successfully! Welcome ${user.name}!`, { id: toastId });
      navigate('/dashboard');
    } catch (error) {
      console.error('Registration error:', error);
      toast.error(error.response?.data?.message || 'Registration failed. Email might be registered.', { id: toastId });
    } finally {
    ...
    <div className="flex items-center justify-center min-h-[70vh] py-6">
      <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-slate-900 bg-slate-900/10 space-y-6 shadow-2xl">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-500 text-xs">Sign up below to build and optimize resumes</p>
        </div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full bg-slate-900/60 border ${
                  errors.name ? 'border-red-500/70' : 'border-slate-800 focus:border-blue-500'
                } rounded-lg pl-10 pr-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:ring-0 transition-colors`}
              />
              <User size={14} className="absolute left-3.5 top-3.5 text-slate-600" />
            </div>
            {errors.name && <p className="text-[10.5px] text-red-500 font-medium">{errors.name}</p>}
          </div>
